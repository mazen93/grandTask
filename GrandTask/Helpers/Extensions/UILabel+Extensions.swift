...
//  AbsherInterior
//
//  Created by AboNabih on 9/23/21.
//

import Foundation
import UIKit

private var setBeforLoc = false

extension UILabel {
    public var setBefore: Bool {
        get {
    ...
    @objc public var defaultFont: UIFont? {
        get { return self.font }
        set {
            guard self.font != nil && setBefore == false else {
                return
            }
...
//            print("oldFontName = \(oldFontName)")
            if oldFontName == ".SFUI-Regular" {
                let arabicFont = UIFont(name: "Cairo-Regular", size: oldFontSize)
                
                if newValue != nil {
                    let englishFont = UIFont(name: "Cairo-Regular", size: oldFontSize)
                    self.font = arabicFont
                }
            }
            else if oldFontName == ".SFUI-Semibold" {
                let arabicFont = UIFont(name: "Cairo-SemiBold", size: oldFontSize)
                
                if newValue != nil {
                    let englishFont = UIFont(name: "Cairo-SemiBold", size: oldFontSize)
                    self.font = arabicFont
                }
                
            }
            else if oldFontName == ".SFUI-Bold" {
                let arabicFont = UIFont(name: "Cairo-Bold", size: oldFontSize)
                
                if newValue != nil {
                    let englishFont = UIFont(name: "Cairo-Bold", size: oldFontSize)
                    self.font = arabicFont
                }
...
extension UILabel {
    ...
    func highlightLabel(searchText: String, color: UIColor = .yellow) {
